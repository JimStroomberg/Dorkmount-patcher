...
The compiler-free runtime data contains 301 replacement bytes generated from the exact reproduced DMR2 outputs: original extension code plus short routing/navigation patches. Complete input/output hashes, bounded ranges and source hashes are checked. No original Clock function or full vendor image is included. The generator is `tools/generate_payload.py`.
