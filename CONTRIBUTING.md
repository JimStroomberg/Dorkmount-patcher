...
DMR1/DMR2 builders are preserved unchanged. Reproduce DMR2 with the documented LLVM toolchain and user-supplied stock files, then run `python3 tools/generate_payload.py <verified-build-directory>` to regenerate the compiler-free patch data. This checks complete hashes, preimages and all changed-byte ranges before exporting only replacement patches. Both generation and runtime patching must retain full input/output hash checks, lengths and range guards.
