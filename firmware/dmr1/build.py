#!/usr/bin/env python3
"""Reproduce the installed DMR1 image from exact user-supplied stock firmware.
...
def build(main, dock, numpad, output):
    ...
    with tempfile.TemporaryDirectory(prefix=".dmr1-build-", dir=output.parent) as temporary:
        ...
        output.mkdir()
        for name, raw in candidates.items():
            (output / f"{name}-dmr1.bin").write_bytes(raw)
...
def main():
    ...
    try:
        ...
        parser.exit(1, f"DMR1 build refused: {error}\n")
